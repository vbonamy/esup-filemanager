...
( function($) {
    ...
    var esupStockFixBlockSizes =  false;
 ...
 */
    ...
    function handleLeftTreeSelection(treeNode) {
        ...
        $.ajax({
            ...
            success: function (r) {
                ...
                if (type != "root" && type != "category" && type != "link" && !needsAuthentication) {
    ...
    function handleFolderDoubleClick(elemClicked, event) {
        ...
        var parentPath = elemClicked.attr('parent');
        if(parentPath) {
            var parentNode = getLiNodeFromPath(parentPath);
            $("#fileTree").jstree("open_node", parentNode, function () {
                var nodeToSelect = getLiNodeFromPath(path);
    ...
    function initDetailsArea() {
        ...
        $('#detail-view-image').bind('click', function () {
            show_image(image_width, image_height);
        });

        // details_sound
        if($("source.audioSource").length) {
    ...
    function show_image(image_width, image_height) {
        ...
        divDialog.attr("title", /*[[#{details.image.title}]]*/ "");
        ...
        var dialogButtons = {};
        dialogButtons[/*[[#{details.images.ok}]]*/ ""] = function () {
            $(this).dialog("close");
        };
